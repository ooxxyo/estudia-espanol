...
import { auditEvent, cleanText, json, listRows, paginateRows, readBody, validDate } from './_shared/platform.mjs';
...
export default async (req) => {
  try {
    ...
    const url = new URL(req.url); const status = cleanText(url.searchParams.get('status'), 30); const type = cleanText(url.searchParams.get('type'), 40); const subject = cleanText(url.searchParams.get('subject'), 40); const author = cleanText(url.searchParams.get('author'), 80); const date = validDate(url.searchParams.get('date'));
    ...
    ].filter(row => (!status || row.status === status) && (!type || row.queueType === type) && (!subject || row.subjectId === subject) && (!author || row.authorId === author) && (!date || row.date === date)).sort((a, b) => b.createdAt - a.createdAt);
    const page = paginateRows(items, url.searchParams);
    return json({ items: page.items, nextCursor: page.nextCursor, limit: page.limit, counts: { contributions: contributions.length, proposals: proposals.length, reports: reports.length } });
  } catch (error) {

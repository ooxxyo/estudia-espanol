...
import { cleanText, isBlockedBetween, json, listRows } from './_shared/platform.mjs';
...
export default async (req) => {
  try {
    ...
    if (auth.ok) {
      const contributions = await listRows(COMMUNITY, 'contributions/', 100);
      for (const row of contributions) {
        if (row.removed || (row.classGroupId && row.classGroupId !== classGroupId) || (!row.classGroupId && classGroupId) || (schoolYearId && row.schoolYearId !== schoolYearId) || (termId && row.termId !== termId) || ['rejected', 'duplicate'].includes(row.status) || !matches(row, query) || (row.authorId !== auth.user.id && await isBlockedBetween(auth.user.id, row.authorId))) continue;
      ...
      const [events, assignments] = await Promise.all([listRows(CALENDAR, 'events/', 100), listRows(ASSIGNMENTS, 'assignments/', 100)]);
      results.push(...events.filter(row => !row.removed && (!(row.classGroupId) ? !classGroupId : row.classGroupId === classGroupId) && (!schoolYearId || row.schoolYearId === schoolYearId) && (!termId || row.termId === termId) && matches(row, query)).map(row => ({ type: 'calendar', id: row.eventId, title: cleanText(row.title, 140), description: 'Evento del calendario', subjectId: row.subjectId, date: row.date })));
      results.push(...assignments.filter(row => !row.removed && (!(row.classGroupId) ? !classGroupId : row.classGroupId === classGroupId) && (!schoolYearId || row.schoolYearId === schoolYearId) && (!termId || row.termId === termId) && matches(row, query)).map(row => ({ type: 'assignment', id: row.assignmentId, title: cleanText(row.title, 140), description: 'Assignment', subjectId: row.subjectId, date: row.dueDate || row.assignedDate })));
    }
    return json({ results: results.slice(0, 100), authenticated: auth.ok });
  } catch (error) {

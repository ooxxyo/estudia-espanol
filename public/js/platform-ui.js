(() => {
  ...
  async function renderCommunity(main, context) {
    ...
    main.innerHTML = `<div class="pagehead"><h1>Comunidad</h1><p>Aportes de estudiantes, separados claramente del material oficial.</p></div>
      ...
      <form class="card platform-filters" id="communityFilters"><label>Materia<select name="subject">${subjectOptions(context, true)}</select></label><label>Fecha<input name="date" type="date" value="${dateLocal()}"></label><label>Tipo<select name="type"><option value="">Todos</option>${['class_notes','assignment','quiz','test','announcement','study_material','other'].map(type => `<option value="${type}">${typeLabels[type]}</option>`).join('')}</select></label><label>Grupo<select name="classGroupId" id="communityGroup"><option value="">General</option></select></label><div class="btn-row"><button class="icon-btn" type="button" data-date-offset="0">Hoy</button><button class="icon-btn" type="button" data-date-offset="-1">Ayer</button><button class="icon-btn" type="submit">Aplicar</button></div></form>
    ...
    const showList = async todayOnly => {
      ...
      try {
        ...
        const card = row => `<article class="card platform-card"><div class="system-row"><div><span class="community-label ${row.status === 'official' ? 'official' : ''}">${context.escape(contributionLabels[row.status] || 'Aporte comunitario')}</span><h3>${context.escape(row.title)}</h3><div class="meta">${context.escape(typeLabels[row.type] || 'Otro')} · ${context.escape(row.date)} · ${context.subjectName(row.subjectId)}</div></div></div><details><summary>Ver detalle</summary><p class="platform-copy">${context.escape(row.text)}</p><p class="cloud-note">${context.escape(row.authorDisplayName)} · ${context.escape(contributionLabels[row.status] || row.status)}</p></details><div class="btn-row"><button class="icon-btn" data-confirm="${row.contributionId}" ${row.own ? 'disabled' : ''}>Sí, esto también lo dieron · ${row.confirmationsCount}</button><button class="icon-btn" data-comments="${row.contributionId}">Comentarios · ${row.commentsCount}</button><button class="icon-btn" data-report="${row.contributionId}">Reportar</button><button class="icon-btn" data-favorite="${row.contributionId}" data-title="${context.escape(row.title)}">♡ Favorito</button></div><div data-comment-panel="${row.contributionId}"></div></article>`;
  ...
  async function renderCalendar(main, context) {
    ...
    main.innerHTML = `<div class="pagehead"><h1>Calendario</h1><p>Eventos escolares y propuestas con permisos separados.</p></div><div class="platform-tabs"><button class="chip active" data-calendar-tab="events">Agenda</button><button class="chip" data-calendar-tab="create">Añadir</button><button class="chip" data-calendar-tab="missed">Falté hoy</button></div><form class="card platform-filters" id="calendarFilters"><label>Materia<select name="subject">${subjectOptions(context, true)}</select></label><label>Tipo<select name="type"><option value="">Todos</option>${['test','quiz','homework','project','presentation','announcement','study','other'].map(type => `<option value="${type}">${typeLabels[type]}</option>`).join('')}</select></label><label>Fecha<input name="date" type="date"></label><label>Grupo<select name="classGroupId" id="calendarGroup"><option value="">General</option></select></label><button class="icon-btn" type="submit">Filtrar</button></form><section id="calendarPanel" aria-live="polite"><div class="empty-state"><span class="ic">…</span>Cargando…</div></section>`;
  ...
  async function renderFriends(main, context) {
    ...
    main.innerHTML = `<div class="pagehead"><h1>Amigos</h1><p>Solicitudes y conexiones con privacidad y bloqueo.</p></div><div class="card"><form class="inline-form" id="profileForm"><label>Buscar username<input name="username" maxlength="20" required></label><button class="icon-btn" type="submit">Ver perfil limitado</button></form><div id="profileResult" aria-live="polite"></div></div><div class="card" style="margin-top:14px"><form class="inline-form" id="friendForm"><label>Username<input name="username" maxlength="20" required></label><button class="btn" type="submit">Enviar solicitud</button></form></div><section id="friendsPanel" aria-live="polite"><div class="empty-state"><span class="ic">…</span>Cargando…</div></section>`;
    ...
    main.querySelector('#profileForm').addEventListener('submit', event => { event.preventDefault(); const form = event.currentTarget; busy(form, 'Buscando…', async () => { const result = main.querySelector('#profileResult'); try { const username = new FormData(form).get('username'); const data = await api(`friends?action=profile&username=${encodeURIComponent(username)}`); const profile = data.profile; result.innerHTML = `<div class="friend-row"><div><b>${context.escape(profile.displayName)}</b><span class="meta">${context.escape(profile.visibleRank || '')}${profile.veteran ? ' · Veterano' : ''} · ${context.escape(profile.relationship)}</span></div></div>`; } catch (error) { result.innerHTML = errorBox(error); } }); });
